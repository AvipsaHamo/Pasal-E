...
export class InventoryComponent implements OnInit {
  ...
  constructor(private inv: InventoryService, private fb: FormBuilder) {}
  ...
  loadCategories(): void {
    this.inv.getCategories().subscribe({ next: c => this.categories = c });
  }
  ...
  onSubmit(): void {
    ...
    const variations = this.showVariations
      ? this.variationsArray.controls
          ...
          .map(c => ({ name: c.get('name')!.value, sellingPrice: c.get('sellingPrice')?.value ?? undefined }))
      : [];
